...
from util_helper_tf1to2 import get_processing_time, pad_arr_seq
...
def get_shuffled_train_data(data_set, data_sample):
    zipped_data = list(zip(data_set, data_sample))

    for k in range(5):
        random.shuffle(zipped_data)

    data_x, data_y = list(zip(*zipped_data))
...
def evaluate_query(q_pred, true_answer, pred_thresh, hits_k, is_analyze):
        pred_list = list(zip(np.arange(len(q_pred)), q_pred))
...
def evaluate_and_record_performance(data_s_ans, data_t_ans, data_pred_s, data_pred_t, agent, dataset, is_analyze, hits_k, mode):
    ...
    if mode == 'valid':
        for ent in list(per_sample_thresh['ent'].keys()):
            agent.thresh_buff['ent'][ent] = np.mean(per_sample_thresh['ent'][ent])

        for reln in list(per_sample_thresh['rel'].keys()):
    ...
    if len(MRR_rel_s) > 0:
        ...
        avg_MRR = np.mean(list(temp_MRR_rel.values()))
    else:
        avg_MRR = -1.0
