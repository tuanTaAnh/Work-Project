...
class DistMult(object):
    ...
    def train_layer(self):
        ...
        pred_score_pos = tf.tile(self.pos_score, [1, self.max_neg_sample])  # None x 7
    ...
    def entity_embedding_layer(self, is_trainable):
        with tf.compat.v1.variable_scope("entity_embedding", reuse=tf.compat.v1.AUTO_REUSE):
            embedding_encoder = tf.compat.v1.get_variable("entity_embedding", [self.max_num_entities, self.embedding_dim],
                                trainable=is_trainable, initializer = tf.keras.initializers.GlorotNormal())
    ...
    def relation_embedding_layer(self, is_trainable):
        with tf.compat.v1.variable_scope("rel_embedding", reuse=tf.compat.v1.AUTO_REUSE):
            embedding_encoder = tf.compat.v1.get_variable("encoder_embedding", [self.max_num_rels, self.embedding_dim],
                                trainable=is_trainable, initializer = tf.keras.initializers.GlorotNormal())
        return embedding_encoder

    def build_network(self):
        # categorical input
        self.s_ent = tf.compat.v1.placeholder(shape=[None, 1], dtype=tf.int32, name='pos_h')
    ...
    def prediction_layer(self):
        ...
        self.pred_scores_s = tf.reshape(pred_score_s, [tf.shape(input=pred_score_s)[0], tf.shape(input=pred_score_s)[1]])  # None x #c

        # reshaping and tiling neg_t prediction ..
        t_q_emb_vec = self.s_ent_emb * self.r_rel_emb  # None x 1 x 3
        t_pred_score = tf.matmul(t_q_emb_vec, cand_ent_emb_tiled, transpose_b=True)
                                                              # None x 1  x 3 *  None x #c  x 3 ---> None x 1 x #c
        self.pred_scores_t = tf.reshape(t_pred_score, [tf.shape(input=t_pred_score)[0], tf.shape(input=t_pred_score)[2]])  # None x #c
