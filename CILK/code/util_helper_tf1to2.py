...
def load_Simulated_User(controller):
    ...
    if controller.KB_name == 'Wordnet_':
        path = '/content/resource_5/user_wordNet0.txt'
    elif controller.KB_name == 'Freebase_':
        path = '/content/resource_5/user_freebase0.txt'
    elif controller.KB_name == 'Conceptnet_':
        path = '/content/resource_5/user_conceptnet0.txt'
    elif controller.KB_name == 'Nell_':
        path = '/content/resource_5/user_nell.txt'
...
def load_test_dataset(agent):
    ...
    if agent.controller.KB_name == 'Wordnet_':
        test_path = '/content/resource_5/test_wordNet0.txt'
        ...
        test_path = '/content/resource_5/test_nell.txt'
...
def get_train_valid_stream(KB):
    ...
    train_stream = []
    for dict_key in list(KB.train_data.keys()):
    ...
    valid_stream = []
    for dict_key in list(KB.valid_data.keys()):
        if dict_key.endswith('-R'):
...
def get_neg_sample_dom(s_q_instance, t_q_instance, r_rel, KB, dom_flag= True): # False = more ram
...
def save_perf_thresh_buffers(KB_name, perf_buff, thresh_buff):
    with open('./KB_dumps/' + KB_name + 'perf_buff.pickle', 'wb') as out_file:
        pickle2.dump(perf_buff, out_file)
    with open('./KB_dumps/' + KB_name + 'thresh_buff.pickle', 'wb') as out_file:
