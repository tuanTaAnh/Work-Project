...
def get_sampled_dataset(input_data, q_ent, q_rel, training_mode, KB, sample_size, mode):
    ...
    if training_mode == 'random':
        all_instances = []
        for dict_key in list(input_data.keys()):
        ...
        if q_rel+'-R' in input_data:
             ...
             sampled_trip = random.sample(filtered_rel_triples, int(min(len(filtered_rel_triples), (sample_size - len(sampled_ent_triples)))))
