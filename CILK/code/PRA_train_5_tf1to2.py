...
from DistMult_5_tf1to2 import *
import random
import pickle as pickle2
from batch_training_helper import *
from util_helper_tf1to2 import *
...
class agent():
    ...
    def initialize_controller(self):
        ...
        print('rel Vocab: ', sorted(list(self.KB.rel_vocab.items()), key=lambda x: x[1]))
        ...
        self.distmult_model = DistMult(self.sess, self.controller.max_rel_vocab_size, self.controller.max_ent_vocab_size,
                                              self.controller.neg_samples_per_triple, self.controller.embd_dim,
                                                           lamda=self.controller.lamda)
        self.saver = tf.compat.v1.train.Saver()
        load_or_initialize_model(self.sess, self.saver, self.controller.KB_name,
                                 '/content/resource_5/' + self.controller.KB_name + '_DistMult_model/')
    ...
    def initial_training(self):
        ...
        if os.path.isfile('./KB_dumps/' + self.controller.KB_name + "0_init_dataset.pickle"):
            ...
            with open('./KB_dumps/' + self.controller.KB_name + "0_init_dataset.pickle", 'wb') as out_file:
        ...
        # save trained model...
        save_model(self.sess, self.saver, self.controller.KB_name, '/content/resource_5/' + self.controller.KB_name + '_DistMult_model/')
    ...
    def run_eval(self):
        ...
        test_stream = []
        for dict_key in list(test_data_store.keys()):
        ...
        with open('./KB_dumps/'+dec_agent.result_folder+'/' + self.controller.KB_name + 'eval_buff.pickle', 'wb') as out_file:
            pickle2.dump(self.evaluation_buffer, out_file)
        with open('./KB_dumps/'+dec_agent.result_folder+'/' + self.controller.KB_name + 'known_info_map.pickle', 'wb') as out_file:
...
if __name__ == "__main__" :
    dec_agent = agent(is_load = False )  # Load the agent.
